...
(async () => {
  ...
  try {
    ...
    await createConsumer(channel, process.env['RABBIT_ADMIN_GW_QUEUE_PRODUCT_UPDATED'], (message) => {
      io.emit('action', { type: process.env['SOCKET_PRODUCT_UPDATED'], payload: JSON.parse(message) })
    });

    await createConsumer(channel, process.env['RABBIT_ADMIN_GW_QUEUE_PRODUCT_CREATED'], (message) => {
